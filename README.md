...
| Savestate | STATE_VERSION estándar | 1.7.8: + latch del pad 6-botones (1.7.7), + bus de la EEPROM I2C (1.7.8), + bloque de continuidad de audio y tag de layout en offset fijo, + bloque de fase de relojes del 68000 (refresh del bus y deteccion de polling, #118) en offset fijo; un estado sin ese bloque (1.10-r2 o anterior) carga con la fase recompuesta, sin continuidad exacta (`check-state-atim-compat`, con un estado real de r2); carga estados 1.7.7 y rechaza los de otra ABI (`check-state-guard`, `check-state-scd-guard`, `check-state-roundtrip`) |
